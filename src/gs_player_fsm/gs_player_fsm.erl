...
%% States : welcome, enter_pass, create_new, create_new_password, authenticating, playing, quitting


-module(gs_player_fsm).
...
%% states
-export([welcome/2, enter_pass/2, playing/2, create_new_yorn/2, create_new_pass/2, confirm_new_pass/2]).
...
%%--------------------------------------------------------------------
init([]) ->
    error_logger:info_msg("Created player fsm~n", []),
...
enter_pass({SocketPid, data, Data}, State = [{player, PlayerName}]) ->
    ...
    case gs_player_auth:authenticate(PlayerName, Data) of
        ...
        {bad_password, PlayerName} ->
            SocketPid ! {send, "Invalid password for that player!\r\n"},
            SocketPid ! {send, "Welcome to Gridspace!\r\nEnter Login: "},
            {next_state, welcome, []}
    end;
